...
lokJSON = hentrada:READ-JSON("longchar",vlcentrada, "EMPTY").
    ...
    then do: 
            ...
            find first segprestcamp where 
                segprestcamp.tpseguro  = segtipo.tpseguro and
                segprestcamp.categoria = vcatnom and
                segprestcamp.etbcod    = int(ttpedidocartaolebes.codigoLoja)
                no-lock no-error.
            if avail   segprestcamp
            then do:              
                ttsegprestpar.campanhaGratis = trim(string(segprestcamp.ativo,"true/false")).
            end.    
            else do:
                find first segprestcamp where 
                    segprestcamp.tpseguro  = segtipo.tpseguro and
                    segprestcamp.categoria = vcatnom and
                    segprestcamp.etbcod    = 0
                    no-lock no-error.
                if avail segprestcamp
                then do:
                    if segprestcamp.dtivig <> ?
                    then do:
                        if segprestcamp.dtivig <= today 
                        then do:
                            if segprestcamp.dtfvig <> ?
                            then do:
                                if segprestcamp.dtfvig >= today
                                then do:
                                    ttsegprestpar.campanhaGratis = trim(string(segprestcamp.ativo,"true/false")).
                                end.
                            end.
