...
field estado_civil as char /*– novas formatação e organização do conteúdo: Casado | Divorciado | Solteiro | União Estável | Viúvo*/
...
field ultimaAtualizacaoCadastral as char /*– date */
...
then do:
    ...
    ttcliente.inscricaoEstadual = Texto(clien.empresa_inscricaoEstadual).
...
*/
    ...
    ttcliente.ultimaAtualizacaoCadastral =  EnviaData(clien.ultimaAtualizacaoCadastral).
    ttcliente.patrimonio = clien.patrimonio no-error.
    ttcliente.pessoaexposta = string(clien.pessoaexposta,"true/false").
    ttcliente.nomeSocial = clien.nomeSocial no-error.
 
    ttcliente.genero = trim(texto(clien.genero)) no-error. /* helio 290823 - retirar espa�os em branco */
    ttcliente.falecido = string(clien.falecido,"true/false").
    ...
    then.  
    else connect crm -H "sv-mat-db1" -S crm_p2k -N tcp -ld crm no-error. /* helio 28032022 -S */
    if connected("crm")
