...
then do:
    ...
    if ttimpressoras.impressora = "NENHUMA IMPRESSORA ENCONTRADA"
    then do:
      create ttsaida.
      ttsaida.tstatus = 300.
      ttsaida.mensagem = "Impressora nao cadastrada ".

    end.
    else do:
      run  etiqueta/1/geraetiqueta.p ("imprimir",if avail ttimpressoras then ttimpressoras.impressora else "").
