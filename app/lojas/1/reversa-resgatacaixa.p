...
define dataset conteudoSaida for ttreversa, ttitens.
...
if reversaLojas.pid <> 0 and 
   int(ttentrada.pid) <> reversaLojas.pid and 
...
reversaLojas.pid            = int(ttentrada.pid).
reversaLojas.dtalt          = today.
reversaLojas.hralt          = time.

for each reversaprodu of reversalojas no-lock.
    create ttitens.  
    ttitens.sequencial    = reversaprodu.seq.
    ttitens.codigoProduto = reversaprodu.procod.
    ttitens.quantidade    = reversaprodu.qtd.
end.
