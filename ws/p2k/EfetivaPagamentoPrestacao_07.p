...
/*
Pagamento parcial somente no datahub
...
then do.
    for each parcelasPag.        
        ...
        /**
            #4 Cheque sem fundos
        **/    
        if parcelasPag.modalidade = "CHQ" and
           vstatus = "S"
        then do.
            if num-entries(parcelasPag.inf_compl,",") <> 3
            then run erro("INF.COMPL. invalida para CHEQUE").
            else do on error undo.
                vbanco   = int(entry(1, parcelasPag.inf_compl)).
                vagencia = int(entry(2, parcelasPag.inf_compl)).
                vconta   = entry(3, parcelasPag.inf_compl).
            
                find cheque where
                         cheque.cheban = vbanco
                     and cheque.cheage = vagencia
                     and cheque.checon = vconta
                     and cheque.chenum = int(parcelasPag.codigo_contrato)
                    no-lock no-error.
                if not avail cheque
                then run erro("CHEQUE " + string(vbanco) + " " +
                              string(vagencia) + " " + vconta + " " +
                              parcelasPag.codigo_contrato + 
                              " - NAO Encontrado").
                else if cheque.chesit <> "LIB"
                    then run erro("CHEQUE: situacao" + cheque.chesit).
            end.
        end.
...
then
        ...
        **/
        ...
        then do:
            ...
            then do:
                ...
                assign
                    titulo.cxacod   = int(efetivapagamentoPrestacao.numero_pdv)
                     /*#1*/
                    titulo.titsit   = "PAG"
                    
                    /**titulo.titjuro  = 00 **/ /**vvlracre -- PROJETO SALDO CARTEIRA **/
                    /***titulo.titdesc  = vvlrdesc***/
                    /**titulo.titvlpag = 0**/  /**vvlrpago -- PROJETO SALDO CARTEIRA **/
                    titulo.titdtpag = today
                    titulo.etbcobra = int(efetivapagamentoPrestacao.codigo_filial).

                    /* registra as parcelas com pagamento */
                    create ttcashback.
   ...
   /* verifica parcelas com pagamento */
   find first ttcashback no-error.
   if avail ttcashback
   then run /admcom/progr/api/efetivacashbackp.p (int(efetivapagamentoPrestacao.codigo_filial),"LOJ").
