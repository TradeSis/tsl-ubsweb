...
*/
{/u/bsweb/progr/acha.i} /***WS {admcab.i}***/

def input  parameter setbcod         as int.

/*** spcconsulta.p ***/
def input  parameter p-ocor         as char.
def input  parameter p-rec          as recid.
def input  parameter par-param      as char. /* parametros genericos */
...
find clien where recid(clien) = p-rec no-lock no-error.
if not avail clien then return.

varqlog = "/u/bsweb/works/con-spc" + string(clien.clicod) + "." + 
          string(mtime) + ".log".
...
def var vtambuffer as int init 15360. /* se aumentar da problema */
...
/***
if setbcod = 1 /* Homologa */
then sigla-usuario = "drebesecia".
***/
...
then do.
    ...
    if clien.estciv = 2 and
       ...
       clien.prorenda[1] = 0
    then assign
            p-cgccpf = acha("CPF",clien.entrefcom[2])
            v-quem = 2.
...
vdir-work = "/u/bsweb/works/" + lc(p-ocor) + string(clien.clicod).
/***WS
...
/***
else     run spcretorno.p (recid(clien),
                           p-ocor, 
                           vdir-work + ".sai",
                           output par-ok).
***/
