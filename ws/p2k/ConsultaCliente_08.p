        ...
        then do:
            ...
            */
            ...
            then do.
                ...
                vtimeini = mtime. 
                run ./progr/pdv/spcpreconsulta_v03.p /*#9*/ (?, recid(clien),
                                                  output par-spc, 
                                                  output vlibera-cli).
                if par-spc 
                then do:
                    ...
                    run ./progr/pdv/spcconsulta_v03.p /*#9*/ (setbcod,
                                       vcxacod,
                                       recid(clien),
                                       output par-spc,
                                       output spc-conecta).
...
then do:
    ...
    bsxml("codigo_senha","").
    bsxml("valor_limite",string(vVlrLimite - sal-abertopr,"->>>>>>>>>9.99")). /*Conversa com Thalis, dia 03/05/2021 Hélio, eu falei com ela. Ela pediu pra mandar o calculado sem o juros, mesmo o saldo em aberto e o crédito estando "errados". No caso considerar o principal no crédito e no saldo em aberto para o calculado ser sem juros*/
    /*** lim-calculado ***/
...
*/
    ...
    bsxml("optinWhatsApp",string(if clien.optinWhatsApp = ? then no else clien.optinWhatsApp,"Sim/Nao")).
    bsxml("optinSMS"     ,string(if clien.optinSMS      = ? then no else clien.optinSMS,"Sim/Nao")).
    /* helio 22122021 - Cadastro P2k - Campos Optin */
