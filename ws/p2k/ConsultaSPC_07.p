...
then do.
    ...
    then do.
        ...
        /* #2 */
        if vstatus = "S" and
           ConsultaSPC.tipo_pessoa = "cliente"
        then do.
            run cpf.p (clien.ciccgc, output par-ok).
            if not par-ok
            then assign
                    vstatus = "E"
...
*/
    if clien.tippes /* #1 and
       not avail clispc */
    then do.
        run ./progr/pdv/spcpreconsulta_v03.p (?, recid(clien),
                                          output par-spc,
                                          output vlibera-cli).
        if par-spc
        then do.
            run ./progr/pdv/spcconsulta_v03.p (setbcod,
                               int(ConsultaSPC.numero_pdv),
                               recid(clien),
                               output par-spc,
                               output spc-conecta).
            /* #1 */
