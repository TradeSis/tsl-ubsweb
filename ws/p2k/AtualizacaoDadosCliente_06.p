    ...
    then do on error undo:
        ...
        then assign
            ...
            then do:
                ...
                find neuclien where neuclien.cpf = dec(vcgccpf) no-lock no-error.
                if avail neuclien
                then do:
                    if neuclien.clicod <> clien.clicod
                    then do:
                        vstatus = "N".
                        vmensagem_erro = "CPF " + string(atualizacaodadoscliente.cpf) +  
                                         " JA EXISTENTE  NO CLIENTE CODIGO " + string(neuclien.clicod).
                    end.
                end.
                else assign
                        vstatus = "S"
...
then do:
    ...
    then do:
        ...
        then do:
            ...
            /* #2 */
            vstatus = "E".
            vmensagem_erro = "CPF " + clien.CICCGC + " Cadastrado Conta Principal " + string(neuclien.clicod).
        end.
    end.
...
bsxml("numero_pdv",string(atualizacaodadoscliente.numero_pdv)).
    ...
    bsxml("tipo_cadastro",atualizacaodadoscliente.tipo_cadastro).
