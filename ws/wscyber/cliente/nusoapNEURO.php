    ...
    */
	...
	if($headers){
		...
		$headers = "<SOAP-ENV:Header>".$headers."</SOAP-ENV:Header>";
                ...
                $headers = "<soapenv:Header></soapenv:Header>";
	}
	// serialize envelope
	return
...
*/
        ...
        '<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:neur="http://neurotech.com.br/">' //HELIO
	...
	function serializeSchema(){
		...
		*/
		$el = "<$schemaPrefix:schema$attr targetNamespace=\"$this->schemaTargetNamespace\"\n";
                $el = "<$schemaPrefix:schema$attr xmlns:tns=\"$this->schemaTargetNamespace\" xmlns:xs=\"http://www.w3.org/2001/XMLSchema\" targetNamespace=\"$this->schemaTargetNamespace\" version=\"1.0\"\n";
...
class wsdl extends nusoap_base {
	...
	*/
	function serialize($debug = 0)
	{
		...
		$serviceName = "name=\"".$this->serviceName."\"";
		$xml .= "\n<wsdl:definitions $serviceName "; //HELIOWSDL
		...
		if (count($this->bindings) >= 1) {
			...
			foreach($this->bindings as $bindingName => $attrs) {
				...
				foreach($attrs['operations'] as $opName => $opParts) {
					...
					$binding_xml .= "\n" . '    <wsdl:input><soap:body use="' . $opParts['input']['use'] . '"' . $enc_style . '/></wsdl:input>';
					if (isset($opParts['output']['encodingStyle']) && $opParts['output']['encodingStyle'] != '') {
						$enc_style = ' encodingStyle="' . $opParts['output']['encodingStyle'] . '"';
					} else {
						$enc_style = '';
					}
					$binding_xml .= "\n" . '    <wsdl:output><soap:body use="' . $opParts['output']['use'] . '"' . $enc_style . '/></wsdl:output>';
					$binding_xml .= "\n" . '  </wsdl:operation>';
...
if(is_array($value) && empty($value)) {
	$value = "";
							...
							$xml = "<$name$elementNS>$value</$name>"; //HELIO
						}
					} else {
		...
		if ($phpType == 'struct') {
			if (isset($typeDef['typeClass']) && $typeDef['typeClass'] == 'element') {
				...
				if (isset($typeDef['form']) && ($typeDef['form'] == 'qualified')) {
					$nprefix =   "ws"; //HELIO
					$elementNS = " xmlns:".$nprefix."=\"$ns\""; //HELIO
				} else {
					$elementNS = " xmlns=\"\"";
				}
			} else {
			...
			if (is_array($value)) {
				...
				if ($use == 'literal') {
					if ($forceType) {
						...
						//if ($elementName contains "Response") {

						if (strpos($elementName,'Response') !== false) {
							$xml =  "<ws:$elementName>"; //HELIO
							
						} else {

						//$xml = "<HELIO$elementName>"; //HELIO
						$xml = "";
                                                } 
					}
				} else {
				...
				if (isset($typeDef['simpleContent']) && $typeDef['simpleContent'] == 'true') {
					...
					// complexContent
					$xml2 = $this->serializeComplexTypeElements($typeDef, $value, $ns, $uqType, $use, $encodingStyle);
					$xml .= $xml2;
